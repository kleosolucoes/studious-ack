...
//	}
  ...
  $mensagensNaoLidas = $driver->findElements(WebDriverBy::cssSelector("span.OUeyt"));

  if (count($mensagensNaoLidas)) {
	  echo "\n\n ##### Tenho mensagens nao lidas #####";
	  sleep(1);
	  echo "\n Procurando Contatos";
	  if($divListaInfinita = $driver->findElements(WebDriverBy::cssSelector("div._3j7s9"))){
		  echo "\n lista infinita"; 
		  ...
		  foreach ($divListaInfinita as $divParaVerificar) {
			  try{
				  echo "\n #### Texto completo: ".$divParaVerificar->getText();
				  $data = explode("\n", $divParaVerificar->getText());
				  $contadorDeEnters = count($data);
				  echo "\n ##### numero de pulo: ".$contadorDeEnters;

				  if ($contadorDeEnters === 4 || $contadorDeEnters === 5) {
					  $nome = $data[0];
		  ...
		  for($indiceContatos = count($listaDeContatos) - 1; $indiceContatos >= 0; $indiceContatos--){
			  ...
			  $inputFiltro = $driver->findElement(WebDriverBy::cssSelector("input.jN-F5"));
			  ...
			  $encontrado = false;
			  try{
				  $driver->findElement(WebDriverBy::cssSelector("div._2EXPL"))->click();
				  $encontrado = true;
			  ...
			  if($encontrado){
				  ...
				  //        $mensagensRecebidas = $driver->findElements(WebDriverBy::cssSelector("div._3zb-j"));
				  //        sleep(1);
				  //
				  ...
				  //        }
				  $elementoInputMensagem
					  ->sendKeys(array('TESTE UM'));
				  echo "\n Escrito";
				  echo "\n Botao de enviar";
				  $driver->findElement(WebDriverBy::cssSelector("button._35EW6"))->click();
				  sleep(2);
				  echo "\n Enviado";
			  }else{
